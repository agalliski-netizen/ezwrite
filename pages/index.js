...
var BLUE = '#3B82F6'; var BLUE_LIGHT = '#EFF6FF';
var BG = '#FAFAFA';
var WHITE = '#FFFFFF'; var TEXT = '#0F172A'; var TEXT2 = '#475569';
var TEXT3 = '#94A3B8'; var BORDER = '#E2E8F0'; var GREEN = '#16A34A';
var GREEN_LIGHT = '#DCFCE7'; var VERSION_COLORS = ['#3B82F6', '#818CF8', '#A78BFA'];
var RED = '#DC2626'; var RED_LIGHT = '#FEF2F2';
var CARD_SHADOW = '0 1px 3px rgba(15, 23, 42, 0.06), 0 1px 2px rgba(15, 23, 42, 0.04)';
var CARD_SHADOW_HOVER = '0 4px 12px rgba(15, 23, 42, 0.08), 0 2px 4px rgba(15, 23, 42, 0.05)';
...
.ez-generate-btn:not(:disabled):hover { filter: brightness(1.08); box-shadow: 0 4px 14px rgba(59, 130, 246, 0.3); }
.ez-generate-btn:not(:disabled) { transition: filter 0.15s ease, box-shadow 0.15s ease; }
.ez-card { transition: box-shadow 0.15s ease; }
.ez-card:hover { box-shadow: ${CARD_SHADOW_HOVER}; }
`}</style>
<div style={{ maxWidth: '640px', margin: '0 auto', paddingTop: '3.5rem', paddingBottom: '4.5rem' }}>
<div style={{ marginBottom: '2.75rem' }}>
<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
<div style={{ fontFamily: dmSerif.style.fontFamily, fontSize: '2rem', color: TEXT, letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '8px' }}>
...
<h1 style={{ fontSize: '17px', fontWeight: 500, color: TEXT2, marginTop: '10px', lineHeight: 1.5, letterSpacing: '-0.01em' }}>{t.tagline}</h1>
</div>
...
<button onClick={handleInstallClick} style={{ background: BLUE, color: WHITE, border: 'none', borderRadius: '6px', padding: '8px 14px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{isIos ? t.installBtnIos : t.installBtn}</button>
<button onClick={handleInstallDismiss} style={{ background: 'transparent', color: TEXT3, border: 'none', fontSize: '13px', cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{t.installDismiss}</button>
</div>
...
<textarea className="ez-card" style={{ width: '100%', minHeight: '130px', padding: '14px 16px', border: isListening ? '1.5px solid '+RED : '1px solid '+BORDER, borderRadius: '10px', fontSize: '14px', color: TEXT, background: isListening ? RED_LIGHT : WHITE, resize: 'vertical', fontFamily: inter.style.fontFamily, lineHeight: 1.6, boxSizing: 'border-box', boxShadow: CARD_SHADOW }} placeholder={t.placeholder} value={message} onChange={function(e) { setMessage(e.target.value); }} />
</div>
<div style={{ marginBottom: '1.75rem' }}>
...
{TONES.map(function(tk) { var active = tone === tk; return (<button key={tk} onClick={function() { setTone(tk); }} style={{ padding: '7px 14px', borderRadius: '6px', border: active ? '1.5px solid '+BLUE : '1px solid '+BORDER, background: active ? BLUE_LIGHT : WHITE, color: active ? BLUE : TEXT2, fontSize: '13px', fontWeight: active ? 600 : 400, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{t.tones[tk] || tk}</button>); })}
</div>
</div>
<div style={{ marginBottom: '1.75rem' }}>
...
<input type="text" value={langSearch} onChange={function(e) { setLangSearch(e.target.value); }} placeholder={t.searchLanguagePlaceholder} style={{ width: '100%', padding: '8px 10px', border: '1px solid '+BORDER, borderRadius: '6px', fontSize: '13px', color: TEXT, fontFamily: inter.style.fontFamily, boxSizing: 'border-box', outline: 'none', marginBottom: '10px' }} />
...
<button className="ez-generate-btn" style={{ width: '100%', padding: '14px', background: isDisabled ? BORDER : BLUE, color: isDisabled ? TEXT3 : WHITE, border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: 600, cursor: isDisabled ? 'not-allowed' : 'pointer', fontFamily: inter.style.fontFamily, marginTop: '0' }} onClick={handleGenerate} disabled={isDisabled}>{loading ? t.writing : t.btn}</button>
{showPaywall && (<div style={{ background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: '10px', padding: '20px', marginTop: '1rem', textAlign: 'center' }}><div style={{ fontSize: '15px', fontWeight: 600, color: '#92400E', marginBottom: '6px' }}>{t.limitTitle}</div><div style={{ fontSize: '13px', color: '#B45309', marginBottom: '16px', lineHeight: 1.5 }}>{t.limitMsg}</div><div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}><a href={`https://ezwrite.lemonsqueezy.com/checkout/buy/301906ea-f048-4b2b-a72d-0734e8869e4a${userId ? "?checkout%5Bcustom%5D%5Buser_id%5D=" + encodeURIComponent(userId) : ""}&checkout%5Bredirect_url%5D=https://ezwrite.app/?activated=1`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', background: '#F59E0B', color: WHITE, padding: '10px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, textDecoration: 'none' }}>{t.limitBtnMonthly}</a><a href={`https://ezwrite.lemonsqueezy.com/checkout/buy/fbe2aa91-d13b-4663-a043-f73b95f0884d${userId ? "?checkout%5Bcustom%5D%5Buser_id%5D=" + encodeURIComponent(userId) : ""}&checkout%5Bredirect_url%5D=https://ezwrite.app/?activated=1`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', background: WHITE, color: '#92400E', border: '1.5px solid #F59E0B', padding: '10px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, textDecoration: 'none' }}>{t.limitBtnAnnual}</a></div></div>)}
...
{versions && (<div className="ph-no-capture"><div style={{ height: '1px', background: BORDER, margin: '2rem 0' }}></div>{versions.map(function(v, idx) { var color = VERSION_COLORS[idx] || BLUE; var isCopied = copied === idx; return (<div key={idx} className="ez-card" style={{ background: WHITE, border: '1px solid '+BORDER, borderLeft: '3px solid '+color, borderRadius: '12px', padding: '18px', marginBottom: '14px', boxShadow: CARD_SHADOW }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}><span style={{ fontSize: '12px', fontWeight: 700, padding: '3px 8px', borderRadius: '4px', background: color+'18', color: color }}>{t.versionLabel} {v.label}</span><button onClick={function() { handleCopy(v.text, idx); }} style={{ fontSize: '12px', fontWeight: 500, padding: '5px 12px', borderRadius: '6px', border: isCopied ? '1px solid '+GREEN : '1px solid '+BORDER, background: isCopied ? GREEN_LIGHT : WHITE, cursor: 'pointer', color: isCopied ? GREEN : TEXT2, fontFamily: inter.style.fontFamily }}>{isCopied ? t.copied : t.copy}</button></div><p style={{ fontSize: '14px', lineHeight: 1.7, color: TEXT, margin: 0, whiteSpace: 'pre-wrap' }}>{v.text}</p></div>); })}</div>)}
