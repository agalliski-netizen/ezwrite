...
useEffect(function() {
if (showPaywall && posthog) posthog.capture('pricing_viewed', { plan_tier: pricingTier });
}, [showPaywall]);
...
<div className="ph-no-capture" style={{ fontSize: '13px', color: C.TEXT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.message}</div>
...
<textarea className="ez-textarea-input ph-no-capture" style={{ width: '100%', minHeight: '130px', padding: '16px 18px', border: isListening ? '1.5px solid '+C.RED : '1px solid '+C.BORDER, borderRadius: '14px', fontSize: '15px', color: C.TEXT, background: isListening ? C.RED_LIGHT : C.SURFACE, resize: 'vertical', fontFamily: inter.style.fontFamily, lineHeight: 1.65, boxSizing: 'border-box', boxShadow: C.SHADOW }} placeholder={t.placeholder} value={message} onChange={function(e) { setMessage(e.target.value); }} />
</div>
<div style={{ marginBottom: '1.75rem' }}>
{(function() {
...
<input type="text" className="ph-no-capture" value={customTone} onChange={function(e) { setCustomTone(e.target.value); }} placeholder={t.customTonePlaceholder} style={{ width: '100%', padding: '10px 14px', border: '1px solid '+C.BORDER, borderRadius: '8px', fontSize: '13.5px', color: C.TEXT, background: C.SURFACE, fontFamily: inter.style.fontFamily, boxSizing: 'border-box', outline: 'none' }} />
{recentCustomTones.length > 0 && (<div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '8px' }}>
{recentCustomTones.map(function(ct, i) { return (<button key={i} className="ph-no-capture" onClick={function() { setCustomTone(ct); }} style={{ padding: '5px 11px', borderRadius: '6px', border: '1px solid '+C.BORDER, background: 'transparent', color: C.TEXT2, fontSize: '12px', cursor: 'pointer', fontFamily: inter.style.fontFamily, maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ct}</button>); })}
</div>)}
</div>)}
</div>
<div style={{ marginBottom: '1.75rem' }}>
...
{showPaywall && (<div style={{ background: isDark ? 'rgba(245,158,11,0.1)' : '#FFF7ED', border: '1px solid '+(isDark ? 'rgba(245,158,11,0.35)' : '#FED7AA'), borderRadius: '10px', padding: '20px', marginTop: '1rem', textAlign: 'center' }}><div style={{ fontSize: '15px', fontWeight: 600, color: isDark ? '#FBBF24' : '#92400E', marginBottom: '6px' }}>{t.limitTitle}</div><div style={{ fontSize: '13px', color: isDark ? '#FCD34D' : '#B45309', marginBottom: '16px', lineHeight: 1.5 }}>{t.limitMsg}</div><div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}><a href={`https://ezwrite.lemonsqueezy.com/checkout/buy/${CHECKOUT_IDS[pricingTier].monthly}${userId ? "?checkout%5Bcustom%5D%5Buser_id%5D=" + encodeURIComponent(userId) : ""}&checkout%5Bredirect_url%5D=https://ezwrite.app/?activated=1`} target="_blank" rel="noopener noreferrer" onClick={function() { if (posthog) posthog.capture('checkout_clicked', { plan: 'monthly', plan_tier: pricingTier, price: PRICES[pricingTier].monthly }); }} style={{ display: 'inline-block', background: '#F59E0B', color: '#fff', padding: '10px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, textDecoration: 'none' }}>{t.limitBtnMonthlyLabel} → ${PRICES[pricingTier].monthly}/{t.perMonth}</a><a href={`https://ezwrite.lemonsqueezy.com/checkout/buy/${CHECKOUT_IDS[pricingTier].annual}${userId ? "?checkout%5Bcustom%5D%5Buser_id%5D=" + encodeURIComponent(userId) : ""}&checkout%5Bredirect_url%5D=https://ezwrite.app/?activated=1`} target="_blank" rel="noopener noreferrer" onClick={function() { if (posthog) posthog.capture('checkout_clicked', { plan: 'annual', plan_tier: pricingTier, price: PRICES[pricingTier].annual }); }} style={{ display: 'inline-block', background: 'transparent', color: isDark ? '#FBBF24' : '#92400E', border: '1.5px solid #F59E0B', padding: '10px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, textDecoration: 'none' }}>{t.limitBtnAnnualLabel} → ${PRICES[pricingTier].annual}/{t.perMonth}</a></div></div>)}
...
<div style={{ marginTop: '3rem', paddingTop: '1.5rem', borderTop: '1px solid '+C.BORDER, display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', columnGap: '8px', rowGap: '6px', fontSize: '12px', color: C.TEXT3 }}><span style={{ whiteSpace: 'nowrap' }}>Made with <span style={{ color: C.ACCENT, fontWeight: 700 }}>EzWrite</span></span><span style={{ whiteSpace: 'nowrap' }}><span style={{ opacity: 0.4, marginRight: '8px' }}>·</span>Powered by <span style={{ color: '#E86A2D', fontWeight: 600 }}>Claude</span></span><span style={{ whiteSpace: 'nowrap' }}><span style={{ opacity: 0.4, marginRight: '8px' }}>·</span><a href="/blog" style={{ color: C.TEXT3, textDecoration: 'underline' }}>Blog</a></span><span style={{ whiteSpace: 'nowrap' }}><span style={{ opacity: 0.4, marginRight: '8px' }}>·</span><a href="/privacidad" style={{ color: C.TEXT3, textDecoration: 'underline' }}>{uiLang === 'Espanol' ? 'Privacidad' : uiLang === 'Portugues' ? 'Privacidade' : 'Privacy'}</a></span></div>
<div style={{ marginTop: '0.9rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '14px' }}>
<a href="https://instagram.com/ezwrite.app" target="_blank" rel="noopener noreferrer" aria-label="Instagram" style={{ color: C.TEXT3, display: 'inline-flex', opacity: 0.75, transition: 'opacity .15s ease' }} onMouseEnter={function(e) { e.currentTarget.style.opacity = 1; }} onMouseLeave={function(e) { e.currentTarget.style.opacity = 0.75; }}>
<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4.2"/><circle cx="17.3" cy="6.7" r="1.15" fill="currentColor" stroke="none"/></svg>
</a>
<a href="https://x.com/Polaco23456" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)" style={{ color: C.TEXT3, display: 'inline-flex', opacity: 0.75, transition: 'opacity .15s ease' }} onMouseEnter={function(e) { e.currentTarget.style.opacity = 1; }} onMouseLeave={function(e) { e.currentTarget.style.opacity = 0.75; }}>
