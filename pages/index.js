...
var THEME = {
light: { BG: '#FAFAFA', SURFACE: '#FFFFFF', SURFACE2: '#F5F6F8', BORDER: '#E2E8F0', TEXT: '#0F172A', TEXT2: '#475569', TEXT3: '#94A3B8', ACCENT: '#3B5BDB', ACCENT_HOVER: '#3350C4', ACCENT_SOFT: 'rgba(59,91,219,0.08)', SHADOW: '0 1px 3px rgba(15,23,42,0.06), 0 1px 2px rgba(15,23,42,0.04)', SHADOW_HOVER: '0 4px 12px rgba(15,23,42,0.08), 0 2px 4px rgba(15,23,42,0.05)', GLOW: '0 4px 14px rgba(59,91,219,0.25)', GREEN: '#16A34A', GREEN_LIGHT: '#DCFCE7', RED: '#DC2626', RED_LIGHT: '#FEF2F2' },
dark: { BG: '#0A0B0F', SURFACE: '#131620', SURFACE2: '#181C28', BORDER: '#242938', TEXT: '#EDEEF2', TEXT2: '#9AA0AE', TEXT3: '#5B6172', ACCENT: '#5B7FFF', ACCENT_HOVER: '#6E8FFF', ACCENT_SOFT: 'rgba(91,127,255,0.12)', SHADOW: '0 1px 3px rgba(0,0,0,0.3), 0 1px 2px rgba(0,0,0,0.2)', SHADOW_HOVER: '0 4px 16px rgba(0,0,0,0.4)', GLOW: '0 4px 18px rgba(91,127,255,0.35)', GREEN: '#34D399', GREEN_LIGHT: 'rgba(52,211,153,0.15)', RED: '#F87171', RED_LIGHT: 'rgba(248,113,113,0.12)' }
};
var VERSION_COLORS_LIGHT = ['#3B5BDB', '#6366F1', '#8B5CF6'];
var VERSION_COLORS_DARK = ['#5B7FFF', '#818CF8', '#A78BFA'];
...
var s23 = useState(false); var isDark = s23[0]; var setIsDark = s23[1];
...
useEffect(function() {
try { var saved = localStorage.getItem('ezw_theme'); if (saved === 'dark') setIsDark(true); } catch(e) {}
}, []);
...
<meta name="theme-color" content={isDark ? '#0A0B0F' : '#FAFAFA'} />
<meta name="description" content={t.tagline} />
</Head>
<style>{`
@keyframes pulse { 0%,100% { opacity:1; } 50% { opacity:0.3; } }
.ez-generate-btn:not(:disabled):hover { filter: brightness(1.08); box-shadow: ${C.GLOW}; }
.ez-generate-btn:not(:disabled) { transition: filter 0.15s ease, box-shadow 0.15s ease, transform 0.1s ease; }
.ez-generate-btn:not(:disabled):active { transform: scale(0.98); }
.ez-card { transition: box-shadow 0.15s ease, border-color 0.15s ease; }
.ez-card:hover { box-shadow: ${C.SHADOW_HOVER}; }
.ez-example-chip { transition: all 0.15s ease; }
.ez-example-chip:hover { border-color: ${C.ACCENT} !important; background: ${C.ACCENT_SOFT} !important; }
...
<div style={{ fontFamily: inter.style.fontFamily, fontWeight: 800, fontSize: '1.5rem', color: C.TEXT, letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '8px' }}>
<span style={{ width: '8px', height: '8px', borderRadius: '50%', background: C.ACCENT, display: 'inline-block' }}></span>
EzWrite
</div>
<div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
<button onClick={toggleTheme} className="ez-theme-toggle" aria-label="Toggle theme" style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', border: '1px solid '+C.BORDER, background: 'transparent', cursor: 'pointer', color: C.TEXT2 }}>
...
<h1 style={{ fontSize: '20px', fontWeight: 600, color: C.TEXT, marginTop: '14px', lineHeight: 1.45, letterSpacing: '-0.01em' }}>{t.tagline}</h1>
</div>
...
{examples.map(function(ex, i) { return (<button key={i} className="ez-example-chip" onClick={function() { handleExampleClick(ex); }} style={{ padding: '8px 14px', borderRadius: '8px', border: '1px solid '+C.BORDER, background: 'transparent', color: C.TEXT2, fontSize: '13px', fontWeight: 500, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{ex.label}</button>); })}
</div>
</div>)}
<div style={{ marginBottom: '1.75rem' }}>
<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
<span style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: C.TEXT3 }}>{t.messageLabel}</span>
...
<textarea className="ez-card" style={{ width: '100%', minHeight: '130px', padding: '14px 16px', border: isListening ? '1.5px solid '+C.RED : '1px solid '+C.BORDER, borderRadius: '10px', fontSize: '15px', color: C.TEXT, background: isListening ? C.RED_LIGHT : C.SURFACE, resize: 'vertical', fontFamily: inter.style.fontFamily, lineHeight: 1.6, boxSizing: 'border-box', boxShadow: C.SHADOW }} placeholder={t.placeholder} value={message} onChange={function(e) { setMessage(e.target.value); }} />
</div>
<div style={{ marginBottom: '1.75rem' }}>
<span style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: C.TEXT3, marginBottom: '8px', display: 'block' }}>{t.recipientLabel}</span>
<div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: recipient === 'Other' || recipient === 'Otro' || recipient === 'Outro' ? '8px' : '0' }}>
{t.recipientOptions.map(function(r) { var active = recipient === r; return (<button key={r} onClick={function() { setRecipient(active ? '' : r); if (!active) setRecipientOther(''); }} style={{ padding: '7px 14px', borderRadius: '6px', border: active ? '1.5px solid '+C.ACCENT : '1px solid '+C.BORDER, background: active ? C.ACCENT_SOFT : 'transparent', color: active ? C.ACCENT : C.TEXT2, fontSize: '13px', fontWeight: active ? 600 : 400, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{r}</button>); })}
</div>
...
<div style={{ marginBottom: '1.75rem' }}>
<span style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: C.TEXT3, marginBottom: '8px', display: 'block' }}>{t.toneLabel}</span>
<div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
{TONES.map(function(tk) { var active = tone === tk; return (<button key={tk} onClick={function() { setTone(tk); }} style={{ padding: '7px 14px', borderRadius: '6px', border: active ? '1.5px solid '+C.ACCENT : '1px solid '+C.BORDER, background: active ? C.ACCENT_SOFT : 'transparent', color: active ? C.ACCENT : C.TEXT2, fontSize: '13px', fontWeight: active ? 600 : 400, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{t.tones[tk] || tk}</button>); })}
</div>
</div>
<div style={{ marginBottom: '1.75rem' }}>
<span style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: C.TEXT3, marginBottom: '8px', display: 'block' }}>{t.langLabel}</span>
<div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
{LANGUAGES.map(function(l) { var active = language === l; return (<button key={l} onClick={function() { setLanguage(l); setShowMoreLanguages(false); }} style={{ padding: '7px 14px', borderRadius: '6px', border: active ? '1.5px solid '+C.ACCENT : '1px solid '+C.BORDER, background: active ? C.ACCENT_SOFT : 'transparent', color: active ? C.ACCENT : C.TEXT2, fontSize: '13px', fontWeight: active ? 600 : 400, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{t.langs[l] || l}</button>); })}
{(function() { var isExtraActive = OUTPUT_LANGUAGES_EXTRA.indexOf(language) !== -1; return (<button onClick={function() { setShowMoreLanguages(!showMoreLanguages); }} style={{ padding: '7px 14px', borderRadius: '6px', border: isExtraActive ? '1.5px solid '+C.ACCENT : '1px dashed '+C.BORDER, background: isExtraActive ? C.ACCENT_SOFT : 'transparent', color: isExtraActive ? C.ACCENT : C.TEXT2, fontSize: '13px', fontWeight: isExtraActive ? 600 : 400, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{isExtraActive ? (t.langs[language] || language) : t.moreLanguages}</button>); })()}
</div>
{showMoreLanguages && (<div style={{ marginTop: '10px', padding: '12px', border: '1px solid '+C.BORDER, borderRadius: '8px', background: C.SURFACE2 }}>
...
{OUTPUT_LANGUAGES_EXTRA.filter(function(l) { return (t.langs[l] || l).toLowerCase().indexOf(langSearch.toLowerCase()) !== -1; }).map(function(l) { var active = language === l; return (<button key={l} onClick={function() { setLanguage(l); setShowMoreLanguages(false); setLangSearch(''); }} style={{ padding: '6px 12px', borderRadius: '6px', border: active ? '1.5px solid '+C.ACCENT : '1px solid '+C.BORDER, background: active ? C.ACCENT_SOFT : 'transparent', color: active ? C.ACCENT : C.TEXT2, fontSize: '13px', fontWeight: active ? 600 : 400, cursor: 'pointer', fontFamily: inter.style.fontFamily }}>{t.langs[l] || l}</button>); })}
</div>
</div>)}
</div>
...
<button className="ez-generate-btn" style={{ width: '100%', padding: '15px', background: isDisabled ? C.BORDER : C.ACCENT, color: isDisabled ? C.TEXT3 : '#fff', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: 600, cursor: isDisabled ? 'not-allowed' : 'pointer', fontFamily: inter.style.fontFamily, marginTop: '0' }} onClick={handleGenerate} disabled={isDisabled}>{loading ? t.writing : t.btn}</button>
<div style={{ textAlign: 'center', marginTop: '10px', fontSize: '12px', color: C.TEXT3 }}>{t.freeNote}</div>
...
{versions && (<div className="ph-no-capture"><div style={{ height: '1px', background: C.BORDER, margin: '2rem 0' }}></div>{versions.map(function(v, idx) { var color = VERSION_COLORS[idx] || C.ACCENT; var isCopied = copied === idx; return (<div key={idx} className="ez-card" style={{ background: C.SURFACE, border: '1px solid '+C.BORDER, borderLeft: '3px solid '+color, borderRadius: '12px', padding: '18px', marginBottom: '14px', boxShadow: C.SHADOW }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}><span style={{ fontSize: '12px', fontWeight: 700, padding: '3px 8px', borderRadius: '4px', background: color+'22', color: color }}>{t.versionLabel} {v.label}</span><button onClick={function() { handleCopy(v.text, idx); }} style={{ fontSize: '12px', fontWeight: 500, padding: '5px 12px', borderRadius: '6px', border: isCopied ? '1px solid '+C.GREEN : '1px solid '+C.BORDER, background: isCopied ? C.GREEN_LIGHT : 'transparent', cursor: 'pointer', color: isCopied ? C.GREEN : C.TEXT2, fontFamily: inter.style.fontFamily }}>{isCopied ? t.copied : t.copy}</button></div><p style={{ fontSize: '14px', lineHeight: 1.7, color: C.TEXT, margin: 0, whiteSpace: 'pre-wrap' }}>{v.text}</p></div>); })}</div>)}
